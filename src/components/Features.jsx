...
const BentoTilt = ({ children, className = "" }) => {
  ...
  const handleMouseMove = (e) => {
    ...
    const tiltX = (relativeX - 0.5) * amplifier;
    const tiltY = (relativeY - 0.5) * -amplifier;

    const newTransform = `perspective(700px) rotateX(${tiltX}deg) rotateY(${tiltY}deg) scale3d(0.98 , 0.95 , 0.95)`;
...
const BentoCard = ({ vid, description, containerClass, title }) => {
  return (
    <div className="relative size-full rounded-md overflow-hidden border-hsla">
      <video
        src={vid}
        autoPlay
        loop
      ...
      <div className="container mx-auto px-3 md:px-10">
        <div className="px-5 py-32">
          ...
          <p className="max-w-md font-circular-web text-lg text-blue-50 opacity-50">
            ...
            vibrant array of products into an interconnected overlay experience
            on your world.
          </p>
        </div>
        <BentoTilt className="relative mb-7 h-96 w-full overflow-hidden rounded-md md:h-[65vh]">
          <BentoCard
            vid={"/videos/feature-1.mp4"}
            title={
              <>
                radi<b>n</b>t
              </>
